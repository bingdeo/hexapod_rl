...
<div style="text-align: center;">
  <img src='Picture/2D Model.png' width='45%' style='display:inline-block; margin-right:5px;'/>
  <img src='Picture/3D Model(origin).png' width='40%' style='display:inline-block;'/>
</div>

---

#### 1-2. Motor & DCMotorCfg
<br>
...
#### 1-3. Sensor & Reward Components
<br>
<div style="text-align: center;">
  <img src='Picture/Component.png' width='50%' style='display:inline-block; margin-right:5px;'/>
</div>
...
<br>
<div style="text-align: center;">
  <img src='Picture/first speed.png' width='50%' style='display:inline-block; margin-right:5px;'/>
</div>
...
<br>
<div style="text-align: center;">
  <img src='Picture/final speed.png' width='50%' style='display:inline-block; margin-right:5px;'/>
</div>
...
### Overview
헥사포드 로봇을 직접 제작하여 학습 신경망을 직접 구동해보는 것을 목표로 한다.
...
1. Motor : MG996R 18개
2. Encoder : AS5600 18개
3. Embedded Board : Arduino Uno/Mega, Jetson Nano
4. Etc : DC-DC Step-Down Converter, Multiplexer(TCA9548A), Li-Po Battery
...
<br>
<div style="text-align: center;">
  <img src='Picture/hardware architecture.png' width='50%' style='display:inline-block; margin-right:5px;'/>
</div>
...
### Edit
시뮬레이션 모델의 관측 공간에 맞게 하드웨어 제원의 수정 과정을 거쳤다.

#### Modeling
1. 신경망 이식을 위해서 오픈 소스 DIY 모델에 존재하지 않던 아두이노 우노와 잿슨 나노 보드를 추가로 부착하였다. 또한 부착 공간을 확보하기 위해 2층을 추가 설치하였다.
2. 각 모터 프레임과 적절한 거리에 엔코더를 부착할 수 있도록 프레임 모델링을 수정하였다.

#### New Item
1. 모터의 Target Position 값을 받아오기 위해 엔코더를 18개 부착하였다. 
2. 엔코더의 IP 주소 분할을 위해 3개의 Multiplexer를 부착하였다.
<br>
...
#### Encoder
모터에 일정한 각도를 명령으로 보내고 엔코더로 그 각도를 측정함으로써 Input Space에 사용된 위치 값과 현실에서 측정한 모터 값의 차이가 생기는 원인을 분석하였다.
오차의 원인은 모터의 급격한 방향 전환 혹은 White Noise가 아닌 엔코더와 모터 간 기계적 정렬 오차에 의한 것임을 확인하였다.
<br>
<div style="text-align: center;">
  <img src='Picture/cal1.png' width='45%' style='display:inline-block; margin-right:5px;'/>
  <img src='Picture/cal2.png' width='40%' style='display:inline-block;'/>
</div>
<br>

#### IMU
가만히 두어도 각속도가 0이 아닌 값을 출력하는 현상이 발생하여 방향성 보정을 수행하였다.
가만히 두었을 때의  출력 데이터를 수집하여 평균을 이용해 자이로 센서로 보정하였고, 특정한 값의 편차가 어느 방향이든 존재함을 발견하였다.
자이로 값과 마찬가지로 지자기 센서의 출력 값을 이용해 편향을 구하고 평균으로 보정하여 해결하였다.
...
<br>
<div style="text-align: center;">
  <img src='Picture/final model.png' width='50%' style='display:inline-block; margin-right:5px;'/>
</div>
